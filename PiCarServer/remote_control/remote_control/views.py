...
def run(request):
    global SPEED, bw_status, q
    debug = ''
    if 'action' in request.GET:
        action = request.GET['action']
        
        # ============== MATLAB Commands =============
        if action == 'MATLAB_SET_CMD':
                        bw.stop()
                        fw.turn(90)
                        print('x         = "%s"' % request.GET['x'])
                        print('y         = "%s"' % request.GET['y'])
                        print('car_angle = "%s"' % request.GET['car_angle'])
                        print('fw_vel    = "%s"' % request.GET['fw_vel'])
                        print('fw_angle  = "%s"' % request.GET['fw_angle'])
                        print('acc       = "%s"' % request.GET['acc'])
                        q.put(request)
        elif action == 'MATLAB_RUN_CMD':
                while not q.empty():
                        req = q.get()
                        print('vel = "%s"' % req.GET['fw_vel'])
                        print('angle = "%s"' % req.GET['fw_angle'])
                        speed = int(req.GET['fw_vel'])
                        if speed >= 0:
                                bw.speed = speed
                                bw.forward()
                                bw_status = 1
                        ...
                        fw.turn(int(req.GET['fw_angle']))
                        time.sleep(0.1)
                print('End! Buffer Cleared!')
                q = queue.Queue(1000)
                bw.stop()
                fw.turn(90)
